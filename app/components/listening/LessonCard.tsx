...
export default function LessonCard({
  ...
  return (
    <Link href={`/features/listening/${id}`}>
      <div className="group h-full bg-[#1e293b] rounded-2xl border border-slate-700/50 shadow-lg shadow-black/20 hover:shadow-2xl hover:shadow-black/40 hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-pointer relative">
        <div className="p-6 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 group-hover:bg-blue-500/30 group-hover:scale-110 transition-all duration-300">
              <SoundOutlined className="text-lg" />
            </div>
          </div>

          {/* Title */}
          <h3 className="text-xl font-bold text-white mb-4 line-clamp-2 group-hover:text-blue-400 transition-colors flex-1">
            {name}
          </h3>
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="flex items-center gap-3 text-sm text-slate-400">
              ...
              <span className="text-slate-300">{getLanguageLabel(language)}</span>
            </div>
          </div>

          {/* Footer Action */}
          <div className="pt-4 border-t border-slate-700 flex items-center justify-between text-sm font-semibold text-white mt-auto">
            <span className="group-hover:text-blue-400 transition-colors">Bắt đầu luyện nghe</span>
            <svg 
